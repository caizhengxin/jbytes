...
pub struct CountExample {
    // try_count  - Specifies the number of Vec elements, if the number of elements is insufficient, no parser error is returned.
    // byte_count - Fetches 2 byte mapping string length in advance.
    #[jbytes(try_count=10, byte_count=2)]
    pub value: Vec<String>,
}


#[test]
fn test_modifier_try_count_example() {
    let bytes = Bytes::new([
...
#[test]
fn test_modifier_try_count_enum_example() {
    let bytes = Bytes::new([
        0x01,                   // branch_value
        0x00, 0x02,             // byte_count, string_len=2,
        0x31, 0x32,             // "12"
